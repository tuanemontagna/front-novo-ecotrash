...
export default function AdminArtigosPage() {
  ...
  function openCreate() {
    setEditing(null);
    setForm({ titulo: "", conteudo: "", publicado: false });
    setModalOpen(true);
  }
  function openEdit(a) {
    setEditing(a);
    setForm({ titulo: a.titulo||"", conteudo: a.conteudo||"", publicado: !!a.publicado });
    setModalOpen(true);
  }
  async function load() {
    try {
      ...
      setArtigos(Array.isArray(res?.data?.data) ? res.data.data : []);
    } catch (e) { setError('Falha ao carregar artigos.'); } finally { setLoading(false); }
  }
  useEffect(()=>{ load(); }, []);
  ...
  async function save(e){
    ...
    try {
      ...
      const payload = { ...form };
      if (editing) {
        await api.patch(`/artigos/${editing.id}`, payload);
      ...
      setModalOpen(false);
      await load();
    } catch (e) { setError(e?.response?.data?.message || 'Erro ao salvar artigo.'); } finally { setSaving(false); }
  }
  async function excluir(){
    if (!confirmDel) return;
    try {
      setError(""); setSuccess("");
      await api.delete(`/artigos/${confirmDel.id}`);
      setSuccess('Artigo excluído.');
      setConfirmDel(null);
      await load();
    } catch (e){ setError(e?.response?.data?.message || 'Erro ao excluir artigo.'); }
  }

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-[#2d5016] text-xl font-semibold">Artigos</h1>
        ...
        <table className="w-full text-sm text-zinc-800">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-4 py-6 text-center text-zinc-500">Nenhum artigo cadastrado.</td></tr>
            ) : (
              artigos.map(a => (
                <tr key={a.id} className="border-t border-zinc-100 hover:bg-zinc-50">
                  <td className="px-4 py-2">{a.id}</td>
                  <td className="px-4 py-2">{a.titulo}</td>
                  <td className="px-4 py-2">{a.publicado? 'Sim':'Não'}</td>
                  <td className="px-4 py-2">
                    <div className="flex gap-2">
                      <button onClick={()=>openEdit(a)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-zinc-300 text-zinc-700 text-xs font-bold hover:bg-zinc-50"><Pencil size={14}/>Editar</button>
                      <button onClick={()=>setConfirmDel(a)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-red-300 text-red-700 text-xs font-bold hover:bg-red-50"><Trash2 size={14}/>Excluir</button>
                    </div>
                  </td>
                </tr>
              ))
      ...
      <Modal open={modalOpen} title={editing? 'Editar artigo':'Novo artigo'} onClose={()=>setModalOpen(false)}>
        <form onSubmit={save} className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-[#2d5016] mb-1">Conteúdo</label>
              <textarea value={form.conteudo} onChange={e=>setForm(f=>({...f,conteudo:e.target.value}))} className="w-full border border-zinc-300 rounded-lg px-3 py-2 text-sm min-h-[160px]" />
            </div>
            <label className="flex items-center gap-2 text-xs font-medium text-[#2d5016]">
              <input type="checkbox" checked={form.publicado} onChange={e=>setForm(f=>({...f,publicado:e.target.checked}))} /> Publicado
            </label>
          </div>
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button disabled={saving} type="submit" className={`px-4 py-2 rounded-lg text-white text-sm font-bold ${saving? 'bg-zinc-400':'bg-[linear-gradient(135deg,#48742c_0%,#5d8f3a_100%)] shadow'}`}>{saving? 'Salvando...':(editing?'Salvar alterações':'Criar artigo')}</button>
          </div>
        </form>
      </Modal>
      ...
      {confirmDel && (
        <Modal open={!!confirmDel} title="Confirmar exclusão" onClose={()=>setConfirmDel(null)}>
          <p className="text-sm text-zinc-700 mb-4">Excluir artigo <strong>{confirmDel.titulo}</strong>?</p>
